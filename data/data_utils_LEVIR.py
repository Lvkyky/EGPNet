...
class CDDataset(ImageDataset):
    ...
    def __init__(self, root_dir, img_size, split='train', is_train=True, label_transform=None,to_tensor=True,Edge=False,Label1=False):
    ...
    def __getitem__(self, index):
        ...
        L1_path = get_label1_path(self.root_dir, self.img_name_list[index % self.A_size])
        ...
        [img, img_B], [label,edge,label1] = self.augm.transform([img, img_B], [label,edge,label1], to_tensor=self.to_tensor)
        # print(label.max())

        # return {'name': name, 'A': img, 'B': img_B, 'L': label, }
        label = label.view(label.shape[1], label.shape[2])

        if self.Edge == True:
            if self.split == 'test':
                return img_original, img_B_original, img, img_B, label, edge

            else:
                return img, img_B, label, edge

        elif self.Label1 == True:
            if self.split == 'test':
                return img_original, img_B_original, img, img_B, label, label1

            else:
                return img, img_B, label, label1

        else:
            if self.split == 'test':
                return img_original, img_B_original, img, img_B, label
